...
void RefracRelPoseEstimator::Estimate(const std::vector<X_t>& points1,
  ...
  virtual_proj_centers1.reserve(num_points);
  virtual_proj_centers2.reserve(num_points);
  ...
  Eigen::MatrixXd A(num_points, 18);
  A.setZero();
  for (size_t i = 0; i < num_points; ++i) {
    const Eigen::Matrix3d eye3 = Eigen::Matrix3d::Identity();
    const Eigen::Vector3d& r1 = points1[i].ray_in_virtual;
    const Eigen::Matrix<double, 1, 3> r1_t = r1.transpose();
    const Eigen::Matrix<double, 1, 3> r2_t = points2[i].ray_in_virtual.transpose();
    ...
    const auto kron1 = Eigen::KroneckerProduct<Eigen::Matrix<double, 1, 3>,
                                               Eigen::Matrix<double, 3, 3>>(r1_t, eye3);
    const Eigen::Matrix<double, 1, 3> cv1_u = (vc1_hat * r1).transpose();
    const auto kron2 = Eigen::KroneckerProduct<Eigen::Matrix<double, 1, 3>,
  ...
  const Eigen::MatrixXd AR = A.block(0, 0, num_points, 9);
  const Eigen::MatrixXd AE = A.block(0, 9, num_points, 9);
  const Eigen::MatrixXd ARpinv = AR.completeOrthogonalDecomposition().pseudoInverse();
  Eigen::MatrixXd eye_n(num_points, num_points);
  eye_n.setIdentity();
  const Eigen::MatrixXd B = (AR * ARpinv - eye_n) * AE;
  const Eigen::JacobiSVD<Eigen::MatrixXd> svd(B, Eigen::ComputeFullV);

  const Eigen::MatrixXd sol = svd.matrixV().col(8);
  const Eigen::Map<const Eigen::Matrix3d> E_raw(sol.data());

  const Eigen::JacobiSVD<Eigen::Matrix3d> E_raw_svd(
  ...
  for (size_t i = 0; i < R_cmbs.size(); ++i) {
    ...
    const double max_depth =
        1000.0 * (R_cmbs[i].transpose() * t_cmbs[i]).norm();
    ...
    for (size_t j = 0; j < num_points; ++j) {
      ...
      const Eigen::Matrix3x4d virtual_proj_matrix1 = virtual_from_real1.ToMatrix();
      const Eigen::Matrix3x4d virtual_proj_matrix2 = virtual2_from_world.ToMatrix();
      Eigen::Vector3d point3D;
      if (!TriangulatePoint(virtual_proj_matrix1,
    ...
    if (points3D_cmb.size() >= points3D.size()) {
      ...
      points3D = std::move(points3D_cmb);
    }
  }
  ...
  const Eigen::JacobiSVD<Eigen::MatrixXd> t_svd(
